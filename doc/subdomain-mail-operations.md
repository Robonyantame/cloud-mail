...
1. 使用现有管理员账号登录，打开“用户列表”。
2. 找到归属用户，例如 `root@tame.cc.cd`，选择“操作 → 邮箱”。
3. 点击“添加子域邮箱”，选择完整子域 `shop.tame.cc.cd`，填写前缀 `root`。也可以每行填写一个前缀（最多 100 个），或切换到随机生成并填写数量。
4. 点击“添加”，确认结果为“已创建”。`root@shop.tame.cc.cd` 归所选用户所有，不会增加登录用户；返回现有收件箱即可选择新地址查看邮件。然后从外部发信验证。

此入口只对站点管理员开放，使用现有登录状态，不需 Public API 令牌。目标普通用户仍受完整子域权限、有效状态和邮箱配额限制。未配置可用子域时窗口会显示提示。用户邮箱列表会区分本功能地址的停用状态；状态修改仍可使用下面的管理 API。

网络故障或返回处理中时，窗口保留原请求并提供“重试本次创建”；同一浏览器标签页刷新后也可恢复，避免随机地址重复生成。超过 24 小时会提示先核对用户邮箱列表再重新填写。不要在结果不明确时另开窗口重复提交新批次。

从上一版更新本网页入口时，须同时发布重新构建的前端静态资源和新版 Worker；不新增数据库迁移。可在 `mail-vue` 目录执行 `pnpm install`、`pnpm run build`，构建输出位于 `mail-worker/dist`，再沿用自己的发布流程。发布后刷新页面再检查入口。原版数据库仍需完成前面的迁移。本次仅进行本地验证，未执行发布。

本次验证通过 29 项 Worker 测试、Worker 打包检查及前端构建，并在独立本地数据库通过网页完成自定义和随机批量创建、确认列表刷新。现有 Vite 7 与 Vue 插件 5 的 peer dependency 提示及大文件分包提示仍存在，构建成功；未修改项目依赖版本。
